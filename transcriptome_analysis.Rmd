...
sdat0 <- read_xlsx("data/sample_metadata.xlsx") %>%
...
ggsave(countfig, filename = "output/countfig.png", device = "png", width = 150, height = 85, units = "mm")
```
...
######
## testing andrew's suggestion of removing three co-dominant C/D samples: 20-12(3), 26-07(25), 20-01(1))
#hs.dds <- hs.dds[, -c(1,3,25)]
...
dapc <- dapc %>%
  ...
  summarize(c1 = mean(LD1),
            c2 = mean(LD2)) %>%
  full_join(dapc)
ggplot(dapc, aes(x = LD1, y = LD2, color = grp)) + geom_point()
...
knitr::kable(DEtab, caption = "Number of differentially expressed genes within individual genets and across all genets for each specified contrast. DEGs identified using DESeq with an adjusted p-value < 0.1. In brackets are the numbers of significantly up- and down-regulated genes.")
...
dek
#save_kable(dek, file = "output/Table1.html")
```


##### Assess similarity of responses across colonies and contrasts
```{r DEgenes_comparison}
# Find genes that are DE in same direction in multiple colonies when analyzed individually
DE %>%
  unnest(sig) %>%
  filter(colony != "all") %>%
  group_by(num, den) %>%
...
KOGDcCc <- KOGheatmap(kogtables$Dc.Cc, kogpvals$Dc.Cc, main = "                     D-control vs. C-control", 
                      fontsize = 8)
ggsave(filename = "figures/FigS2.png", plot = KOGDcCc, width = 112, height = 84.5, units = "mm")
KOGChCc <- KOGheatmap(kogtables$Ch.Cc, kogpvals$Ch.Cc, main = "                     C-heated vs. C-control",
                      fontsize = 8)
ggsave(filename = "figures/FigS3.png", plot = KOGChCc, width = 112, height = 84.5, units = "mm")
KOGDhDc <- KOGheatmap(kogtables$Dh.Dc, kogpvals$Dh.Dc, main = "                     D-heated vs. D-control",
                      fontsize = 8)
ggsave(filename = "figures/FigS4.png", plot = KOGDhDc, width = 112, height = 84.5, units = "mm")
KOGDhCh <- KOGheatmap(kogtables$Dh.Ch, kogpvals$Dh.Ch, main = "                     D-heated vs. C-heated",
                      fontsize = 8)
ggsave(filename = "figures/FigS5.png", plot = KOGDhCh, width = 112, height = 84.5, units = "mm")
```
...
# Get number of genes DE p<0.05 associated with each KOG term
kogout <- KOG %>%
  unnest(KOG) %>%
  filter(colony == "all") %>%
  unite("contrast", num, den, sep = ".") %>%
...
kogout <- kogout %>%
  mutate(nsig = map2_dbl(term, dsr, function(.x, .y) {
    geneswithterm <- filter(gene2kog, V2 == .x) %>% pull(V1)
...
fig4 <- plot_grid(NULL, grob4a, fig4b, rel_widths = c(0.025, 0.685, 0.29), nrow = 1, labels = c("A", "", "B"))
...
ggsave(noshuffpcoa, file = "output/noshuffpcoa.png", width = 50, height = 50, units = "mm")
```
