...
ggplot(df, aes(x = trt1, y = propD, color = trt1), labeller = global_labeller) +
  ...
  labs(x = "", y = "Proportion Durusdinium") +
  theme_custom() +
...
## Proportion D
```{r}
# Corals that were maintained at 26°C throughout the study
df %>%
  filter(trt1 == "c", trt == "ctrl") %>%
  summarise(meanC = mean((1 - propD), na.rm = TRUE),
            sd = sd(1 - propD, na.rm = TRUE))

# Corals that shuffled to D after initial bleaching (colonies 20, 22, 26, trt1=="b")
df %>%
  filter(colony %in% c(20, 22, 26), trt1 == "b") %>%
  summarise(meanD = mean(propD),
            sd = sd(propD))

# Corals that did NOT SHUFFLE to D after initial bleaching (colonies 20, 22, 26, trt1=="b")
df %>%
  filter(!colony %in% c(20, 22, 26), trt1 == "b") %>%
  summarise(meanC = mean(1 - propD),
            sd = sd(1 - propD))
